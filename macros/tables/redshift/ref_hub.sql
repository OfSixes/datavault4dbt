{%- macro redshift__ref_hub(ref_keys, src_ldts, src_rsrc, source_models) -%}
...
distinct_target_ref_keys AS (
    SELECT{{ datavault4dbt.concat_ws(ref_keys) }}
    FROM {{ this }}
),
...
rsrc_static_union AS (
    ...
    {%- set source_number = source_model.id | string -%}
    SELECT rsrc_static_{{ source_number }}.*
...
ref_key_ordering AS (
    {#- Add ordering per ref_key to unionized records #}
    SELECT
        lcte.*,
        ROW_NUMBER() OVER (PARTITION BY {% for ref_key in ref_keys -%} {{ref_key}} {%- if not loop.last %}, {% endif -%} {%- endfor %} ORDER BY {{ src_ldts }}) AS {{ rdshft_order_col }}
    FROM {{ ns.last_cte }} AS lcte

    {%- set ns.last_cte = 'ref_key_ordering' %}
),
...
records_to_insert AS (
    ...
    SELECT
        {{ datavault4dbt.print_list(final_columns_to_select, 0) | indent(8) }}
    FROM {{ ns.last_cte }}
{%- if is_incremental() %}
