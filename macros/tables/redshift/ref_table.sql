...
{%- if historized in ['full', 'latest'] -%}
    ...
    SELECT MAX({{ date_column }}) AS {{ date_column }}
    FROM (
    {%- endif -%}
...
ref_table AS (
    SELECT
        {{ datavault4dbt.print_list(list_to_print=ref_key_cols, indent=, src_alias='h') | indent(8) }},
        ld.{{ date_column }},
        h.{{ src_rsrc }},
{%- for satellite in ref_satellites_dict.keys() -%}
    ...
    {{- log('sat_columns: '~ sat_columns, false) }}
        {{ datavault4dbt.print_list(list_to_print=sat_columns, indent=0, src_alias=sat_alias) | indent(8) -}}
    {%- if not loop.last -%},{%- endif -%}
{%- endfor %}
    FROM {{ ref(ref_hub) }} AS h
    FULL JOIN dates AS ld
        ON TRUE
{%- for satellite in ref_satellites_dict.keys() -%}
    {%- set sat_alias = 's_' + loop.index|string %}
    LEFT JOIN {{ ref(satellite) }} AS {{ sat_alias }}
