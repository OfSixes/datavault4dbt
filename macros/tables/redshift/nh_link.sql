...
src_new_{{ source_number }} AS (
    ...
    {%- endfor %}
        {{ src_ldts }},
        {{ src_rsrc }},
        {{ datavault4dbt.print_list(source_model['payload'], 0) | indent(8) }}
...
hk_ordering AS (
    {#- Add ordering per hashkey to unionized records #}
    SELECT
        lcte.*,
        ROW_NUMBER() OVER (PARTITION BY {{ link_hashkey }} ORDER BY {{ src_ldts }} ASC) AS {{ rdshft_order_col }}
    FROM {{ ns.last_cte }} AS lcte

    {%- set ns.last_cte = 'hk_ordering' %}
),
