{%- macro redshift__ma_sat_v0(parent_hashkey, src_hashdiff, src_ma_key, src_payload, src_ldts, src_rsrc, source_model) -%}
...
duplicate_marked_hashdiff AS (
    SELECT
        {{ parent_hashkey }},
        {{ src_ldts }},
        {{ ns.hdiff_alias }},
        CASE
            WHEN {{ ns.hdiff_alias }} = LAG({{ ns.hdiff_alias }}) OVER (PARTITION BY {{ parent_hashkey }} ORDER BY {{ src_ldts }})
                THEN TRUE
                ELSE FALSE
...
deduped_row_hashdiff AS (
    SELECT
        ...
        {{ src_ldts }},
        {{ ns.hdiff_alias }}
    FROM duplicate_marked_hashdiff
...
deduped_rows AS (
    SELECT
        ...
        {{ datavault4dbt.alias_all(columns=source_cols, prefix='source_data') | indent(8) }}
    FROM source_data
...
records_to_insert AS (
    SELECT
        ...
        {{ datavault4dbt.alias_all(columns=source_cols, prefix='deduped_rows') | indent(8) }}
    FROM deduped_rows
{%- if is_incremental() %}
    WHERE NOT EXISTS (
        SELECT 1
