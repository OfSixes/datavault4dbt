{%- macro redshift__sat_v1(sat_v0, hashkey, hashdiff, src_ldts, src_rsrc, ledts_alias, add_is_current_flag, include_payload) -%}
...
end_dated_source AS (
    ...
    {%- if include_payload -%},
        {{ datavault4dbt.print_list(source_columns_to_select, 0) | indent(8) }}
    {%- endif %}
...
{%- if add_is_current_flag %},
    CASE
        WHEN {{ ledts_alias }} = {{ datavault4dbt.string_to_timestamp(timestamp_format, end_of_all_times) }}
            THEN TRUE
            ELSE FALSE
    END AS {{ is_current_col_alias }}
{%- endif -%}
{%- if include_payload %},
    {{ datavault4dbt.print_list(source_columns_to_select, 0) | indent(4) }}
{%- endif %}
