...
{%- macro redshift__hash_method() %}
    ...
    {%- if global_var is mapping -%}
        {%- if 'redshift' in global_var.keys()|map('lower') -%}
            {% set hash_method = global_var['redshift'] %}
        {%- else -%}
            {%- if execute -%}
                {%- do exceptions.warn("Warning: You have set the global variable 'datavault4dbt.hash' to a dictionary, but have not included the adapter you use (redshift) as a key. Applying the default value.") -%}
            {% endif %}
            {%- set hash_method = 'MD5' -%}
        {% endif %}
    {%- elif global_var is not mapping and datavault4dbt.is_something(global_var) -%}
