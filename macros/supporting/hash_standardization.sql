...
{%- macro redshift__attribute_standardise() -%}
    ...
    '"' || REPLACE(REPLACE(REPLACE(TRIM(CAST([EXPRESSION] AS VARCHAR(20000))), '\\', '\\\\'), '[QUOTE]', '"'), '[NULL_PLACEHOLDER_STRING]', '--') || '"'
...
{%- macro redshift__concattenated_standardise(case_sensitive, hash_alg, datatype, zero_key, alias) -%}
    ...
    {%- set hash_alg_prefix = hash_alg ~ "(" -%}
    {%- set hash_alg_suffix = ")" -%}
    {%- if hash_alg == "SHA2" -%}
        {%- set hash_alg_suffix = ", 256)" -%}
    {%- endif -%}

    {%- set standardise_steps = [
        ("TRANSLATE(", ", CHR(10) || CHR(9) || CHR(11) || CHR(13), '')"),
        ("CAST(", " AS VARCHAR(65535))"),
        ("NULLIF(", ", '[ALL_NULL]')"),
        (hash_alg_prefix, hash_alg_suffix),
        ("NVL(", ", {})".format(zero_key)),
     ] -%}
    {%- if case_sensitive -%}
        {%- set standardise_steps = [("UPPER(", ")")] + standardise_steps -%}
    {%- endif -%}
    {%- if alias is not none -%}
        {%- set standardise_steps = standardise_steps + [("", " AS {}".format(alias))] -%}
    {%- endif -%}

    {%- set standardise_prefix = "" -%}
    {%- set standardise_suffix = "" -%}

    {%- for prefix, suffix in standardise_steps -%}
        {%- set standardise_prefix = prefix ~ standardise_prefix -%}
        {%- set standardise_suffix = standardise_suffix ~ suffix -%}
    {%- endfor -%}
