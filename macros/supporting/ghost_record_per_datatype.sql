...
{%- macro exasol__ghost_record_per_datatype(column_name, datatype, ghost_record_type, col_size, alias) -%}
    ...
    {%- if ghost_record_type == 'unknown' -%}
        {%- if datatype == 'TIMESTAMP' or datatype == 'TIMESTAMP WITH LOCAL TIMEZONE' -%}
            {{- datavault4dbt.string_to_timestamp( timestamp_format , beginning_of_all_times) }} as "{{ alias }}"
...
{%- macro redshift__ghost_record_per_datatype(column_name, datatype, ghost_record_type, col_size, alias) -%}
    ...
    {%- if ghost_record_type == 'unknown' -%}
        {%- if datatype.startswith('TIMESTAMP') -%} {{ datavault4dbt.string_to_timestamp(timestamp_format, beginning_of_all_times) }} AS {{ alias }}
        {%- elif datatype == 'DATE' -%} CAST({{ datavault4dbt.string_to_timestamp(timestamp_format, beginning_of_all_times) }} AS DATE) AS {{ alias }}
        {%- elif datatype in ['VARCHAR', 'NVARCHAR', 'TEXT'] -%} '{{ unknown_value__STRING }}' AS {{ alias }}
        {%- elif datatype in ['CHAR', 'NCHAR', 'BPCHAR'] -%} CAST('{{ unknown_value_alt__STRING }}' as {{ datatype }} ) as {{ alias }}
        {%- elif 'VARCHAR(' in datatype -%}
            ...
            {%- else -%}
                CAST('{{ unknown_value__STRING }}' as {{ datatype }} ) as {{ alias }}
            {%- endif -%}
        {%- elif 'CHAR(' in datatype -%} CAST('{{ unknown_value_alt__STRING }}' as {{ datatype }} ) as {{ alias }}
        {%- elif datatype.startswith('INT') or datatype.startswith('FLOAT')
            or datatype in ['SMALLINT', 'BIGINT', 'DECIMAL', 'NUMERIC', 'REAL', 'DOUBLE PRECISION'] -%}
            0 AS {{ alias }}
        {%- elif datatype in ['BOOLEAN', 'BOOL'] -%} CAST('FALSE' AS BOOLEAN) AS {{ alias }}
        {%- else -%} NULL AS {{ alias }}
        {%- endif -%}
    {%- elif ghost_record_type == 'error' -%}
        {%- if datatype.startswith('TIMESTAMP') -%} {{ datavault4dbt.string_to_timestamp(timestamp_format, end_of_all_times) }} AS {{ alias }}
        {%- elif datatype == 'DATE' -%} CAST({{ datavault4dbt.string_to_timestamp(timestamp_format, end_of_all_times) }} AS DATE) AS {{ alias }}
        {%- elif datatype in ['VARCHAR', 'NVARCHAR', 'TEXT'] -%} '{{ error_value__STRING }}' AS {{ alias }}
        {%- elif datatype in ['CHAR', 'NCHAR', 'BPCHAR'] -%} CAST('{{ error_value_alt__STRING }}' as {{ datatype }} ) as {{ alias }}
        {%- elif 'VARCHAR(' in datatype -%}
            {%- set inside_parenthesis =  datatype.split(")")[0] |string -%}
            {%- set inside_parenthesis = inside_parenthesis.split("(")[1] -%}
            {%- set unknown_dtype_length = inside_parenthesis | int -%}
            ...
            {%- else -%}
                CAST('{{ error_value__STRING }}' as {{ datatype }} ) as {{ alias }}
            {%- endif -%}
        {%- elif 'CHAR(' in datatype -%} CAST('{{ error_value_alt__STRING }}' as {{ datatype }} ) as {{ alias }}
        {%- elif datatype.startswith('INT') or datatype.startswith('FLOAT')
            or datatype in ['SMALLINT', 'BIGINT', 'DECIMAL', 'NUMERIC', 'REAL', 'DOUBLE PRECISION'] -%}
            -1 AS {{ alias }}
        {%- elif datatype in ['BOOLEAN', 'BOOL'] -%} CAST('FALSE' AS BOOLEAN) AS {{ alias }}
