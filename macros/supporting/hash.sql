...
{%- macro redshift__hash(columns, alias, is_hashdiff, multi_active_key, main_hashkey_column) -%}
    ...
    {%- for column in columns -%}
        ...
        {%- endif -%}
        {{- "\n" -}}
        {{- "    NVL(({}), '{}')".format(attribute_standardise | replace('[EXPRESSION]', column_str) | replace('[QUOTE]', quote) | replace('[NULL_PLACEHOLDER_STRING]', null_placeholder_string), null_placeholder_string) -}}
        {%- if loop.last -%}
            {{- "\n" -}}
            {{ standardise_suffix | replace('[ALL_NULL]', all_null | join("")) }}
        {%- else -%}
            {{- "\n    || '{}' ||".format(concat_string) -}}
            {%- do all_null.append(concat_string) -%}
        {%- endif -%}
    {%- endfor -%}
